...
function hqBootShell() {
  ...
  hqCached("hq_conn_v1", 60000, function () {
    ...
    if (pill && pillText) {
      ...
      // ПИЛЮЛЯ НАЗЫВАЕТ СВОЙ ИСТОЧНИК. Прежде она говорила просто «Данные
      // актуальны», и на страницах, где рядом живёт ВТОРОЙ источник со своей
      // свежестью, это читалось как утверждение про него. На «Поставках» такой
      // источник есть: снимок производственной таблицы Google Sheets
      // обновляется только по кнопке владельца и к синхронизации МойСклада
      // отношения не имеет. Два разных источника с двумя разными расписаниями
      // не должны отчитываться одной безымянной фразой — иначе ошибка или
      // задержка одного молча выдаётся за состояние другого.
      pillText.textContent = name + " · " + (fresh
        ? "данные актуальны"
        : "данные от " + fmtDate(conn.last_sync_at));
