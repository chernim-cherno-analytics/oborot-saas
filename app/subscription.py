...
Состояния (D-24):
  ...
  readonly — бизнес-данные менять нельзя, читать и экспортировать можно.

Что закрывает readonly:
  1) любые изменения бизнес-данных организации,
  2) синхронизацию и запись в МойСклад,
  3) расчёт и сохранение новых планов заказа.
Чтение прежних данных, экспорт, вход и управление аккаунтом, интерфейсные
предпочтения, страница тарифов и заявка на счёт НЕ закрываются никогда: клиент
с истёкшей подпиской должен видеть свои данные, иметь возможность заплатить и
управлять собственным аккаунтом. Служебный lifecycle МойСклада тоже открыт:
именно через него магазин сообщает об активации подписки. Граница закреплена
тестом-сторожем (tests/test_subscription.py).
...
BLOCK_MESSAGE = (
    "Доступ к изменениям приостановлен: подписка не оплачена. "
    "Прежние данные, отчёты и экспорт открыты; изменения, синхронизация и "
    "новые рекомендации возобновятся после оплаты. "
