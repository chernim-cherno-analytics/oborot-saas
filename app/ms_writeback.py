...
async def push_order(db: Session, org_id: int, order: ProductionOrder,
    ...
    async with MoySkladClient(token) as client:
        ...
        if existing is not None:
            ...
            # Документа ещё нет — сейчас он будет СОЗДАН, и уедет он ровно
            # `positions`, то есть РАЗБИВКА. Если разбивка расходится с общим
            # количеством заказа, подрядчик получил бы одно число, а владелец
            # видел бы у себя другое. Такие строки могли быть сохранены раньше,
            # чем вход API стал их отклонять (`app.api.OrderItemIn`); молча
            # починить их нельзя — какая из двух цифр верна, знает только
            # владелец.
            #
            # Почему проверка ЗДЕСЬ, а не в начале push_order. Выше по функции
            # ещё не известно, создаём мы документ или подбираем уже созданный:
            # это выясняет find_own_document, а он — сетевой. Ранняя проверка
            # закрывала бы и recovered-путь, а он существует ровно для того,
            # чтобы заказ с УЖЕ созданным документом не остался сиротой: там
            # правда берётся из самого документа, а не из локального снимка,
            # и локальное расхождение ему не мешает (DATA-7 Package B,
            # tests/test_writeback_idempotency.py, сценарий 30а). Запретить
            # его значило бы навсегда оставить финансовый документ в чужом
            # аккаунте без владельца — это дороже, чем два лишних GET.
            #
            # Ни одной записи о состоявшейся отправке к этому моменту нет:
            # ключ идемпотентности не израсходован, commit_push ещё впереди.
