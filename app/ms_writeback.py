...
async def resolve_agent(db: Session, org_id: int, client, keys: PushKeys) -> dict:
    ...
    Порядок ровно такой и по одной причине на шаг:
      1) уже закреплённая ссылка — используем её и в сеть за агентом не идём:
         решение про то, КОМУ уходит финансовый документ, принимается один
         раз и не пересматривается при каждой отправке;
