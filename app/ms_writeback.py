...
async def push_order(db: Session, org_id: int, order: ProductionOrder,
    ...
    # Заказ, у которого количество позиции расходится с её размерной
    # разбивкой, наружу не уходит. Такие строки могли быть сохранены раньше,
    # чем вход API стал их отклонять (`app.api.OrderItemIn`), и молча починить
    # их здесь нельзя: документ поставщику собирается ИЗ РАЗБИВКИ
    # (_item_size_breakdown), то есть подрядчик получил бы одно количество, а в
    # заказе осталось бы другое — и разошлось бы это уже деньгами и поставкой.
    #
    # Проверка стоит ДО первого сетевого вызова: ни документа, ни попытки его
    # создать, ни занятого ключа идемпотентности. Правильную цифру система за
    # владельца не выбирает — какое из двух чисел верное, знает только он.
    mismatched: list[str] = []
    for item in order.items:
        ...
        if split != item_qty:
            mismatched.append(
                f"«{item.get('base_name') or '—'}» — в заказе {item_qty} шт, "
                f"по размерам {split} шт")
    if mismatched:
        raise WritebackError(
            409,
            "Заказ не отправлен: количество расходится с разбивкой по размерам ("
            + "; ".join(mismatched)
            + "). Документ поставщику собирается по размерам, поэтому он ушёл бы "
              "не на то количество, которое стоит в заказе. Исправьте заказ и "
              "повторите отправку — выбрать за вас верную цифру мы не вправе.",
        )
