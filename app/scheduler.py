...
это вешает shutdown-хендлер FastAPI; start() main.py вызывает сам, последней
строкой _startup(), после всех миграций (см. attach() ниже — OPS-6). Env
SCHEDULER_ENABLED=0 отключает планировщик (тесты, dev-запуски).
...
def attach(app) -> None:
    """Вешает остановку планировщика на lifecycle FastAPI-приложения.

    Вызывается из main.py: `from app import scheduler; scheduler.attach(app)`.

    Запуск (`start()`) сюда намеренно не входит (OPS-6): FastAPI выполняет
    `on_event("startup")`-хендлеры строго в порядке регистрации, а этот вызов
    случается раньше, чем main.py регистрирует свой `_startup` (init_db +
    ensure_schema всех модулей). Раньше это значило, что планировщик реально
    поднимался ДО того, как гарантированно применены все аддитивные миграции.
    Теперь `start()` вызывается последней строкой `app.main._startup()` —
    после того как все шаги миграции завершились успешно; если любой из них
    бросает исключение, `_startup` не доходит до `start()` вовсе, и
    планировщик не стартует. `shutdown()` регистрации не потребовалось
    менять — порядок остановки этим не затронут.
    """
