...
async def _read_upload(file: UploadFile, limit: int, too_big: str) -> bytes:
    """Читает файл ПОТОКОМ с потолком.

    `Content-Length` приходит от клиента, и верить ему нельзя — файл,
    объявленный маленьким, может оказаться каким угодно. Поэтому потолок
    считается по фактически прочитанным байтам, а чтение обрывается на первом
    же превышении, а не после того, как всё окажется в памяти.

    Текст отказа передаётся вызывающим, а не сочиняется здесь: у полного эскиза
    и у миниатюры пределы разные, и один текст на оба сказал бы человеку
    неправду о том, что именно не влезло.
    """
    data = bytearray()
    while True:
