...
FMT_NUM1 = "#,##0.0"
FMT_NUM2 = "#,##0.00"
#: Количество материала: до ТРЁХ знаков и ни одного лишнего нуля.
#: Слой принимает и хранит 0,001 (`MIN_QTY`), а `#,##0.00` показывал
#: 0,001 и 0,004 как «0,00», то есть ПРЯТАЛ ненулевое количество, а
#: 0,005 показывал как «0,01» — то есть менял его. Значение в ячейке
#: при этом было и остаётся точным: врал только показ (замечание ревью
#: PR #58). `#` — необязательная цифра, поэтому 100 остаётся «100», а
#: 10,5 — «10,5»: третий знак появляется там, где он есть.
FMT_QTY3 = "#,##0.###"
...
#: Те же символы, но проверяемые у ПЕРВОГО НЕПРОБЕЛЬНОГО знака. Пробелы и
#: переводы строк перед формулой Excel и CSV съедают так же, как табуляцию.
_FORMULA_HEADS = ("=", "+", "-", "@")

#: Управляющие символы, которых НЕ БЫВАЕТ в XML 1.0, а значит и в xlsx: книга
#: это ZIP с XML внутри. Ровно этот набор отвергает openpyxl
#: (`ILLEGAL_CHARACTERS_RE`), поднимая `IllegalCharacterError`. `\t` (09),
#: `\n` (0A) и `\r` (0D) в набор НЕ входят и остаются нетронутыми.
_XML_ILLEGAL = re.compile(r"[\x00-\x08\x0b\x0c\x0e-\x1f]")


def _xlsx_safe(value: str) -> str:
    """Текст, пригодный для ячейки. Меняется ТОЛЬКО представление.

    ЗАЧЕМ ЭТО ЗДЕСЬ, А НЕ В ПРОВЕРКЕ ВВОДА. Слой планирования принимает строку
    человека как есть: `clean_text` обрезает края и считает длину, но внутренние
    символы не «чинит» — это его текст, а не наш. Управляющий знак внутри
    названия (обычно он приезжает вставкой из Word или из чужой таблицы)
    сохраняется и живёт в базе, и это не дефект хранения: показать такую строку
    на экране браузер умеет.

    А xlsx — не умеет: книга это XML, и `U+000B` в ней недопустим по стандарту.
    Без этой замены ОДНА принятая строка роняла всю выгрузку организации
    (`IllegalCharacterError`, 500 на кнопке «Скачать xlsx») до тех пор, пока
    человек не найдёт и не отредактирует запись — а найти он её не может,
    потому что символ невидим. Замечание ревью PR #58, воспроизведено HTTP:
    POST 200, затем GET export.xlsx 500.

    ПОЧЕМУ ПРОБЕЛ, А НЕ УДАЛЕНИЕ. Управляющий знак почти всегда стоит там, где
    в исходном документе был перенос или разделитель: «Ткань\\x0bпартия» — это
    два слова, и склеивать их в «Тканьпартия» значило бы менять читаемый текст
    сильнее, чем требуется. Пробел сохраняет границу слов.

    ЧЕГО ЭТА ФУНКЦИЯ НЕ ДЕЛАЕТ: не трогает базу, не отвергает ввод задним
    числом, не нормализует регистр, пробелы, юникод, переводы строк и
    табуляцию. Она существует ровно на границе «наши данные → чужой формат».
    """
...
def _cell(ws: Worksheet, row: int, col: int, value,
    ...
    ПОРЯДОК ДВУХ ЗАЩИТ ЗДЕСЬ ЗНАЧИМ, И ИМЕННО В ЭТОМ. Сначала снимаются
    недопустимые для XML управляющие символы (`_xlsx_safe`), и ТОЛЬКО ПОТОМ
    решается вопрос формулы. Обратный порядок открыл бы дыру, которой раньше не
    было: строка `U+000B` + `=cmd|…` начинается с символа, которого нет в
    `_FORMULA_STARTS`, защиту бы не получила — а после очистки превратилась бы
    в ячейку, начинающуюся с формулы. Поэтому проверяется то, что РЕАЛЬНО ляжет
    в ячейку, а не то, что пришло.

    По той же причине смотрим не только на первый символ, но и на первый
    НЕПРОБЕЛЬНЫЙ: пробел перед «=» Excel и CSV съедают ровно так же, как
    табуляцию, ради которой в `_FORMULA_STARTS` уже стоит `\\t`.
    """
    if value is None:
        value = ""
    if isinstance(value, str):
        value = _xlsx_safe(value)
...
def _sheet_title(wb: Workbook, name: str) -> str:
    ...
    Управляющие символы снимаются той же `_xlsx_safe`, что и содержимое ячеек:
    имя листа лежит в том же XML и недопустимо в нём по той же причине. Здесь
    это предупреждение, а не воспроизведённый отказ — имена листов приходят из
    названий производств, и такого случая я не воспроизводил; но оставлять
    единственную непочищенную дорогу пользовательского текста в книгу значило
    бы починить один вход из двух.
    """
    name = _xlsx_safe(name or "")
    clean = "".join("-" if ch in ':\\/?*[]' else ch for ch in name.strip())
...
def turnover_workbook(org_name: str, data: dict) -> Workbook:
    ...
    ws = _new_sheet(wb, "Оборачиваемость", org_name, len(headers),
                    subtitle="оборачиваемость и дни в стоке — окно 2 года (канон, D-35); деньги и продано — за год · покрытие и стокаут: "
...
def supply_workbook(org_name: str, data: dict) -> Workbook:
    ...
    for m in data.get("materials") or []:
        ...
        ], {2: FMT_QTY3, 3: FMT_QTY3, 4: FMT_QTY3})
        row += 1
    if row == 3:
    ...
    for b in data.get("batches") or []:
        ...
        _write_row(ws, row, [
            ...
            b.get("due_source") or "",
            materials,
        # План изделий — ЦЕЛОЕ число штук по построению (`parse_pieces`),
        # и дробный формат показывал бы «60,00» там, где 60 не бывает
        # дробным.
        ], {3: FMT_INT})
        row += 1
    if row == 3:
