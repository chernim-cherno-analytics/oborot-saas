...
def api_connect_demo(ctx: AuthContext = Depends(require_owner_api), db: Session = Depends(get_db)):
    ...
    # ВТОРОЙ ПРЕДОХРАНИТЕЛЬ: СВОЙ ПЛАН «ПОСТАВОК» НЕ СТИРАЕТСЯ ПЕРВЫМ ДЕМО.
    #
    # Проверка ревью PR #58 (тред 3958649864), воспроизведено синтетически до
    # правки: на организации БЕЗ МойСклада человек заводил материал, вещь и
    # партию руками, нажимал «Демо-данные» — и получал 200, после которого на
    # доске оставались только три посеянных материала. Прежний предохранитель
    # смотрел лишь на живой источник, а ручной план под него не подпадал вовсе:
    # никакого источника у него нет, он и есть источник.
    #
    # ПРИЗНАК — НАЛИЧИЕ ПРЕЖНЕГО ДЕМО-ПОДКЛЮЧЕНИЯ, А НЕ ТЕКСТ В ДАННЫХ. Строка
    # `connections(kind='demo')` создаётся ТОЛЬКО этой ручкой и только после
    # удавшегося посева, то есть её наличие означает «демо здесь уже включали».
    # Поле `author` и любой другой показываемый текст в решении не участвуют:
    # это ввод пользователя, и строить на нём решение о стирании его же данных
    # нельзя — подделывается тривиально.
    #
    # Отсюда ровно три исхода, и все три названы:
    #   * демо здесь уже было — пересев разрешён, это его штатный смысл;
    #   * демо ещё не было и плана нет — первое включение проходит;
    #   * демо ещё не было, а план есть — он не наш, и мы ОТКАЗЫВАЕМ.
    #
    # Отказ стоит ДО первой записи: `seed_demo` начинается с `clear_org_data`,
    # и «проверить после» означало бы проверить по руинам.
    demo_conn = db.execute(
        select(Connection).where(Connection.org_id == org.id,
    ...
    if demo_conn is None:
        ...
        rows = _demo_seed.supply_plan_rows(db, org.id)
        if any(rows.values()):
            raise HTTPException(
                status_code=409,
                detail="В разделе «Поставки» уже есть ваш план — материалы, "
                       "вещи, плановые партии или их история. Демо-данные "
                       "стёрли бы его безвозвратно, поэтому включение "
                       "остановлено и НИЧЕГО не изменено. Уберите свои записи "
                       "сами, если они больше не нужны, или напишите в "
                       "поддержку — поможем безопасно.")
...
# ── Обучение: уроки по страницам (страница /lessons) ─────────────────────────
# Числа уроков здесь намеренно нет: их ровно столько, сколько записей в
# `lessons.CATALOGUE`, и всякий раз, когда оно стояло в комментарии, оно
# расходилось с каталогом при первом же добавлении урока.
#
