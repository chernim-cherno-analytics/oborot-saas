...
class SupplyAssignment(Base):
    ...
    updated_at: Mapped[datetime] = mapped_column(TolerantDateTime, nullable=False,
                                                 default=datetime.utcnow)
    rev: Mapped[int] = mapped_column(Integer, nullable=False, default=1, server_default="1")
    #: Снято вместе со своей партией, но НЕ стёрто (SUPPLY-FIX-2, решение
    #: владельца 5562704475). Пометка, а не `DELETE`, потому что владелец
    #: потребовал возвращать партию ВМЕСТЕ с прежними назначениями и заметками:
    #: восстановить ТУ ЖЕ заметку можно только не потеряв её, а не собирая
    #: обратно из поля журнала на 500 символов.
    #:
    #: Архивное назначение не считается нигде: ни в `assigned`, ни в сводке, ни
    #: на карточках. Поэтому «убрали партию — назначенное у материала
    #: уменьшилось» выполняется, а метраж при этом цел и возвращается целиком.
    archived_at: Mapped[datetime | None] = mapped_column(TolerantDateTime, nullable=True)
...
def ensure_schema(bind=None) -> None:
    ...
    Чего здесь СОЗНАТЕЛЬНО нет: SUPPLY-1 (`ensure_supply_schema`). Эта функция
    вызывается из `db.init_db()` — шага старта, который на боевых базах давно
    записан в журнале под id `init_db` и позицией 1. Дописать сюда новую
    схемную работу значило бы исполнить её под идентичностью УЖЕ ВЫПУЩЕННОГО
    шага: собственного свидетельства о применении в `migration_ledger` у неё
    бы не появилось, и порядок относительно будущих миграций журнал бы не
    удерживал. Это и есть «менять миграции задним числом» (AGENTS.md §1),
    и append-only контракт `STARTUP_SCHEMA_STEPS` в `app/main.py` требует для
    нового смысла новый id и новую позицию. Ревью PR #46,
    discussion_r3894000377.
    """
    eng = bind or engine
...
def ensure_supply_schema(bind=None) -> None:
    """SUPPLY-1 (D-49/D-50): колонка партии, её замок уникальности и backfill.

    ОТДЕЛЬНЫЙ шаг старта, а не часть `ensure_schema()`. Разница не
    косметическая — ревью PR #46 (discussion_r3894000377). Пока эта работа
    исполнялась внутри `ensure_schema()`, она шла под идентичностью уже
    выпущенного шага `init_db` (позиция 1). На базах, где журнал такую строку
    уже содержит, отдельного свидетельства о применении SUPPLY-1 не
    появлялось бы вовсе, а порядок относительно будущих миграций журнал бы не
    удерживал: смысл выпущенного шага менялся бы задним числом. AGENTS.md §1
    отвечает на это одной строкой — «Только новая миграция сверху», — и
    append-only контракт `STARTUP_SCHEMA_STEPS` в `app/main.py` говорит то же
    прямым текстом: новый смысл получает новый id и новую позицию, а прежние
    строки журнала остаются как есть.

    Отсюда и публичность имени: `models.ensure_supply_schema` — это шаг
    объявленного реестра старта, а не деталь реализации `ensure_schema`.

    Функция ИДЕМПОТЕНТНА и рассчитана на вызов НА КАЖДОМ старте. Строка в
    журнале — свидетельство того, что шаг однажды прошёл, и намеренно НЕ
    является основанием его пропустить (`db.validate_migration_step`,
    `main._startup_step`: сначала `run()`, потом запись). Для backfill это
    существенно: рядом с новым процессом какое-то время живёт откатившийся
    старый код, чей INSERT создаёт пустой `cc_batch_id` уже ПОСЛЕ того, как
    шаг записан, — и следующий старт обязан такую строку вылечить
    (см. `backfill_cc_batch_ids`).
    ...
    ход (forward) снова доводит пустые строки до идентификаторов. Старый код
    не знает и про строку журнала — она просто остаётся лежать, а вернувшийся
    новый код видит свою пару (id, позиция) неизменной и продолжает лечить.

    bind — необязательный engine (тестам нужен, чтобы прогнать шаг на
    отдельной базе со «старой» схемой); по умолчанию — engine приложения.
    """
    eng = bind or engine
    insp = inspect(eng)
...
def ensure_supply_planning_schema(bind=None) -> None:
    ...
    ЧТО ЗДЕСЬ ДЕЛАЕТСЯ, И ПОЧЕМУ ЭТОГО ДОСТАТОЧНО. Шесть таблиц и один частичный
...
#: SUPPLY-FIX-1 (F-09, F-10): замки «одна пара — одна строка».
#:
#: Оба индекса УНИКАЛЬНЫЕ и оба созданы отдельным шагом старта, а не дописаны в
#: шаг 11: у выпущенной пары (id, позиция) своё свидетельство в журнале, и
#: исполнять новую работу под ним значило бы, что о ней журнал не знает вовсе.
#:
#: Первый — назначения: один материал на одну плановую партию живёт ОДНОЙ
#: строкой. Индекс полный, без `WHERE`: у назначения нет состояния «черновик»,
#: в котором пара могла бы законно повториться.
_SUPPLY_ASSIGNMENT_PAIR_DDL = (
    "CREATE UNIQUE INDEX IF NOT EXISTS ux_supply_assignments_pair "
    "ON supply_assignments (org_id, material_id, batch_id)"
)
#: Второй — каталожные вещи: одна модель каталога в плане организации заводится
#: один раз. Индекс ЧАСТИЧНЫЙ и это существенно: у новинок (`kind='draft'`)
#: `base_name` пуст у всех сразу, и полный индекс запретил бы вторую новинку
#: вообще — то есть замок против дублей превратился бы в запрет работать. Две
#: новинки с одним рабочим именем по-прежнему две разные вещи (D-49: решение за
#: человеком), и этот индекс их не касается.
_SUPPLY_ITEM_CATALOG_DDL = (
    "CREATE UNIQUE INDEX IF NOT EXISTS ux_supply_items_catalog "
    "ON supply_items (org_id, base_name) WHERE kind = 'catalog'"
)

#: Предел длины заметки — тот же, что у колонки `note` (String(500)). Столько
#: же вмещает одно поле журнала (`supply_events.old_value`), и это не совпадение,
#: а то, на чём держится сохранность при слиянии: ОДНА заметка человека всегда
#: помещается в ОДНО поле журнала целиком, сколько бы их ни склеивали в видимую.
_SUPPLY_NOTE_LIMIT = 500

#: Автор записей журнала, сделанных шагом старта. Не человек и не роль — и
#: подписывать их чьим-то именем значило бы приписать человеку чужой поступок.
_MIGRATION_AUTHOR = "миграция SUPPLY-FIX-1"


def _journal_row(conn, org_id: int, kind: str, entity_id: int, action: str,
                 field: str, old: str, new: str) -> None:
    """Строка журнала из шага старта. Тем же INSERT, что пишет приложение.

    `op_id` пустой намеренно: частичный замок `ux_supply_events_op` считает
    только непустые, поэтому таких строк может быть сколько угодно и они не
    мешают идемпотентности поступков человека.
    """
    conn.execute(text(
        "INSERT INTO supply_events (org_id, entity_kind, entity_id, action,"
        " field, old_value, new_value, author, op_id, created_at)"
        " VALUES (:o, :k, :e, :a, :f, :old, :new, :author, '', datetime('now'))"
    ), {"o": org_id, "k": kind, "e": entity_id, "a": action, "f": field,
        "old": (old or "")[:_SUPPLY_NOTE_LIMIT],
        "new": (new or "")[:_SUPPLY_NOTE_LIMIT],
        "author": _MIGRATION_AUTHOR})


def _join_notes(notes: list[str]) -> tuple[str, bool]:
    """Склейка заметок в одно видимое поле. Возвращает (значение, урезано ли).

    ПОЧЕМУ ЗДЕСЬ ВООБЩЕ ВОЗМОЖНА ОБРЕЗКА. Каждая отдельная заметка человека
    ограничена пятьюстами символами на вводе, а склейка двух таких — нет: 300 и
    300 дают 603 вместе с разделителем и в колонку `note` уже не помещаются.
    Расширять колонку ради этого нельзя (это правка выпущенной схемы), а
    выбрать «первую» — значит молча выбросить вторую.

    Поэтому обрезается ТОЛЬКО ВИДИМОЕ поле, и вызывающий обязан сначала
    положить каждую исходную заметку в журнал целиком: там для каждой из них
    своё поле того же размера, и ни один символ не теряется. Флаг `cut`
    существует ровно для того, чтобы обрезка не осталась молчаливой — по нему
    пишется отдельная запись журнала.
    """
    joined = " · ".join(notes)
    if len(joined) <= _SUPPLY_NOTE_LIMIT:
        return joined, False
    return joined[:_SUPPLY_NOTE_LIMIT], True


def _merge_duplicate_assignments(conn) -> int:
    """Схлопывает дубли (org_id, material_id, batch_id) ПЕРЕД созданием индекса.

    Сумма назначенного — величина, на которую человек смотрит: она обязана
    пережить слияние до последней десятой. Заметки обеих строк писал он же,
    поэтому они склеиваются, а не выбираются одна из двух. Выживает строка с
    наименьшим `id` — та, которую он завёл первой, и на которую уже могли
    сослаться журнал и открытая вкладка.

    НИ ОДНА СТРОКА НЕ УДАЛЯЕТСЯ, ПОКА ЕЁ ТЕКСТ НЕ ЗАПИСАН. Перед каждым DELETE
    в журнал уходит заметка удаляемой строки ЦЕЛИКОМ — по записи на строку,
    поэтому длина склейки на сохранность не влияет вовсе. Если видимая склейка
    в колонку не поместилась, об этом пишется ещё одна запись: обрезка обязана
    быть названной, иначе человек читает укороченный текст как полный (D-37).
    """
    groups = conn.execute(text(
        "SELECT org_id, material_id, batch_id FROM supply_assignments "
        "GROUP BY org_id, material_id, batch_id HAVING COUNT(*) > 1"
    )).all()
    removed = 0
    for org_id, material_id, batch_id in groups:
        rows = conn.execute(text(
            "SELECT id, qty, note FROM supply_assignments "
            "WHERE org_id = :o AND material_id = :m AND batch_id = :b "
            "ORDER BY id"
        ), {"o": org_id, "m": material_id, "b": batch_id}).all()
        keep_id = rows[0][0]
        total = 0.0
        notes: list[str] = []
        for _id, qty, note in rows:
            total += float(qty or 0.0)
            clean = (note or "").strip()
            if clean and clean not in notes:
                notes.append(clean)
        merged_note, cut = _join_notes(notes)
        # Сначала журнал, потом удаление. Порядок здесь не стилистический: всё
        # внутри одной транзакции, и запись, сделанная ПОСЛЕ DELETE, была бы
        # такой же по итогу, — но правило «сначала след, потом потеря» должно
        # быть видно в коде, чтобы его нельзя было потерять при следующей правке.
        for row in rows[1:]:
            donor_note = (row[2] or "").strip()
            _journal_row(conn, org_id, "assignment", keep_id, "merge", "qty",
                         str(row[1]), str(round(total, 3)))
            if donor_note:
                _journal_row(conn, org_id, "assignment", keep_id, "merge", "note",
                             donor_note, merged_note)
            conn.execute(text("DELETE FROM supply_assignments WHERE id = :i"),
                         {"i": row[0]})
            removed += 1
        if cut:
            # Видимое поле короче суммы написанного. Полные тексты уже лежат
            # записями выше — эта строка говорит, что искать их надо там.
            _journal_row(
                conn, org_id, "assignment", keep_id, "merge", "note_truncated",
                str(len(" · ".join(notes))),
...
def _merge_duplicate_catalog_items(conn) -> int:
    """Схлопывает дубли каталожных вещей и ПЕРЕВЕШИВАЕТ их партии на выжившую.

    Порядок здесь единственно возможный: сначала переезжают партии, потом
    исчезает строка. Наоборот — это осиротевший `item_id` в `supply_batches`,
    то есть партия, которую экран не покажет ни под одной вещью, а человек
    считает пропавшей. Новинок (`kind='draft'`) шаг не касается вовсе.

    ЗАМЕТКА ДУБЛЯ — ТОЖЕ ВВЕДЁННЫЙ ТЕКСТ, И ОНА ПЕРЕЕЗЖАЕТ ВМЕСТЕ С ПАРТИЯМИ.
    Прежняя редакция читала у дублей один `id`, переносила партии и удаляла
    строку — вместе с заметкой, которую человек написал именно про эту вещь.
    Теперь заметки склеиваются по тому же правилу, что у назначений, каждая
    удаляемая уходит в журнал целиком до DELETE, а обрезка видимого поля
    называется отдельной записью.
    """
    groups = conn.execute(text(
        "SELECT org_id, base_name FROM supply_items "
        "WHERE kind = 'catalog' AND base_name <> '' "
        "GROUP BY org_id, base_name HAVING COUNT(*) > 1"
    )).all()
    removed = 0
    for org_id, base_name in groups:
        rows = conn.execute(text(
            "SELECT id, note FROM supply_items WHERE org_id = :o "
            "AND kind = 'catalog' AND base_name = :b ORDER BY id"
        ), {"o": org_id, "b": base_name}).all()
        keep_id = rows[0][0]
        notes: list[str] = []
        for _id, note in rows:
            clean = (note or "").strip()
            if clean and clean not in notes:
                notes.append(clean)
        merged_note, cut = _join_notes(notes)
        for row in rows[1:]:
            donor_note = (row[1] or "").strip()
            if donor_note:
        ...
        if cut:
            _journal_row(
                conn, org_id, "item", keep_id, "merge", "note_truncated",
                str(len(" · ".join(notes))),
                "видимая заметка обрезана до предела поля; полные тексты — "
                "в записях этого слияния")
...
def ensure_supply_planning_unique_schema(bind=None) -> None:
    """SUPPLY-FIX-1: дубли схлопываются, замки встают. Шаг старта 12.

    ПОЧЕМУ ОТДЕЛЬНЫЙ ШАГ. По тому же правилу, что шаг 11 у SUPPLY-3 и шаг 10 у
    SUPPLY-1 (AGENTS.md §1 «только новая миграция сверху», ревью PR #46
    discussion_r3894000377): смысл новый — значит и пара (id, позиция) новая.
    Одиннадцать выпущенных пар не тронуты ни одной буквой, иначе старт на боевой
    базе упал бы `MigrationLedgerConflict`, и это не дефект, а замок.

    ЧТО ЗДЕСЬ ДЕЛАЕТСЯ, И В КАКОМ ПОРЯДКЕ. Сначала слияние существующих дублей,
    потом уникальные индексы. Обратный порядок невозможен: индекс на базе с
    дублями не создастся, старт упадёт, и организация с двумя строками «фурнитура»
    осталась бы без приложения целиком.

    ЧТО ЭТОТ ШАГ МЕНЯЕТ В ЧУЖИХ ДАННЫХ — НАЗЫВАЕТСЯ ПРЯМО. Это ПЕРВЫЙ шаг слоя,
    который не только создаёт пустое, но и трогает уже введённые строки. Он
    удаляет строки-дубли, и вот что при этом сохраняется:

      * сумма назначенного по паре (материал, партия) — до десятых;
      * заметки обеих строк — склейкой, а не выбором одной;
      * партии дублирующих каталожных вещей — переносом на выжившую вещь.

    Ничего, кроме дублей, шаг не читает и не пишет: у организаций без дублей он
    не изменит ни одной строки. Идемпотентен — после первого прохода групп с
    `COUNT(*) > 1` не остаётся, и повторный старт делает ноль записей.

    ОТКАТ — ПОСЛЕДСТВИЕ НЕ В ДАННЫХ, НО ОНО ХУЖЕ, ЧЕМ ЗДЕСЬ СНАЧАЛА БЫЛО
    НАПИСАНО. Первая редакция этого абзаца обещала после отката «409 Это
    действие уже выполнено». Число было выведено из чтения кода и оказалось
    неверным. Прогон настоящего старого приложения на мигрированной базе —
    сначала мой на `ea1caff`, затем независимый на фактическом rollback target
    `e7e106ed` — дал ДРУГОЕ: HTTP 500 «Internal Server Error» и
    `sqlite3.IntegrityError: UNIQUE constraint failed`.

    Почему именно 500, а не управляемый отказ: в `create_assignment` до этого
    пакета стоит `db.add(row); db.flush()`, а маршрут
    `api_planning_assignment_create` ловит только `PlanningError`. Индекс
    появился позже прежнего кода, обработчика этой ошибки у него нет и быть не
    могло, поэтому исключение уходит наружу целиком.

    Что при этом ИЗМЕРЕНО и верно: строки после отката целы (в опыте осталась
    ровно одна, qty 70), старый код читает их как свои, доска отвечает 200,
    слитая заметка видна, приложение после отказа живо — на новом соединении
    доска 200, новый материал 200, назначение на ДРУГУЮ пару 200. Отказ
    точечный: он бьёт только по повторному назначению той же пары (и по
    повторной каталожной вещи).

    ЭТО ОТКРЫТЫЙ RELEASE GATE, А НЕ ПРОСТО ЗАПИСЬ. Устранить его правкой
    внутри ЭТОГО пакета нельзя: после отката исполняется прежний код, и любой
    обработчик, добавленный здесь, там отсутствует. Минимальный безопасный
    вариант — выпустить сначала отдельный микропакет совместимости (маршруты
    ловят `IntegrityError` рядом с `PlanningError`, без схемы и без изменения
    поведения на базе без индексов), сделать его rollback target, и только
    потом этот шаг. Это меняет порядок выпуска, поэтому решение за владельцем;
    сам такую перестройку я не делаю.
    ...
    with eng.begin() as conn:
        _merge_duplicate_assignments(conn)
...
def ensure_supply_assignment_archive_schema(bind=None) -> None:
    """SUPPLY-FIX-2: отметка архива у назначения. Шаг старта 14.

    ПОЧЕМУ ОТДЕЛЬНЫЙ ШАГ, А НЕ ДОПИСКА В 13-й. Пятый раз по тому же правилу
    (`AGENTS.md` §1 «только новая миграция сверху»): у нового смысла новая пара
    (id, позиция). Шаг 13 к этому моменту уже мог отработать на живой базе, и
    дописать в него четвёртую таблицу значило бы, что на такой базе колонка не
    появится вовсе — журнал считает шаг выполненным по его id, а не по составу.
    Тринадцать выпущенных пар не тронуты ни буквой.

    ЧТО ЗДЕСЬ ДЕЛАЕТСЯ. Один `ALTER TABLE supply_assignments ADD COLUMN
    archived_at DATETIME`. Ни одной строки шаг не читает и не переписывает, ни
    одного индекса не создаёт и не удаляет — выпущенный `ux_supply_assignments_pair`
    остаётся как есть.

    ЗАМОК ПАРЫ И АРХИВ НЕ КОНФЛИКТУЮТ, и это проверено рассуждением о состояниях,
    а не надеждой. `ux_supply_assignments_pair` полный, без `WHERE`, поэтому
    архивное назначение продолжает занимать свою пару (материал, партия). Второе
    назначение на ту же пару могло бы появиться только через живую партию, но
    архивное назначение существует ТОЛЬКО у архивной партии, а на архивную
    партию назначить нельзя вовсе (`get_batch` её не отдаёт). Пара занята ровно
    одной строкой в любом состоянии, и ослаблять индекс не потребовалось.
    ...
    ОТКАТ. Прежний код колонку не выбирает и не называет в `INSERT`, она
    нуллируема — его запись проходит, строка получает `NULL`, то есть «живая».
    Цена та же, что у шага 13 и названа так же прямо: прежний код по этой
    колонке не фильтрует, поэтому снятые вместе с партией назначения после
    отката снова видны и снова считаются в `assigned`. Данные не теряются и не
    портятся; вернувшийся новый код прячет их обратно.
    """
    eng = bind or engine
    insp = inspect(eng)
    if not insp.has_table("supply_assignments"):
        return
    cols = {c["name"] for c in insp.get_columns("supply_assignments")}
    if "archived_at" not in cols:
        run_migration_step(
