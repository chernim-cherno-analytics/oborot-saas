...
#: Потолок длины поискового запроса по строкам снимка. Поиск читает уже
#: проверенный снимок в памяти и ничего не пишет, но принимать в параметре
#: строку любой длины всё равно нельзя: длина параметра — это чужой ввод, а
#: чужой ввод ограничивается там же, где принимается.
...
#: Предельная вложенность того, что уезжает в ответ.
#:
#: Найдено собственным тестом этого же корректива, а не ревью: обход, который я
#: сделал итеративным ради устойчивости к глубине, устойчивость НЕ ДАЁТ — ниже
#: по течению стоит `jsonable_encoder()` FastAPI, и он рекурсивный. Замерено на
#: этом рантайме: он проходит 500 уровней и падает `RecursionError` на 1000, то
#: есть глубокий, но совершенно ЧИТАЕМЫЙ носитель ронял страницу и без единого
#: суррогата.
#:
#: Число выбрано заведомо низким, и это осознанно. Точного порога у `500…1000`
#: нет: он зависит от того, сколько кадров уже лежит на стеке — middleware,
#: обработчик маршрута, сериализация, — и потому сидеть рядом с ним нельзя.
#: Настоящий снимок имеет глубину около пяти (ответ → строки → строка → карта
#: размеров → значение), так что 64 не задевает ни одних живых данных и снимает
#: зависимость от чужого предела рекурсии целиком.
...
#: Та же шестёрка кодов, но словами: ЧТО именно не прочитано и какое действие
#: разумно. Подпись из `ISSUE_LABELS` называет случай, а этот текст его
#: объясняет — на экране они стоят рядом, поэтому и живут рядом здесь.
#:
#: Границы текстов заданы жёстко и не смягчаются. Ни один из них не обещает
#: срыва производства, не назначает срока и не требует переписать чужую
#: таблицу: раздел читает источник, а не управляет им. «Неизвестная колонка» —
#: это заявление о НАШЕМ незнании, а не претензия к тому, кто эту колонку
#: завёл; текст говорит прямо, что портить из-за неё источник не нужно.
#: Про местоположение здесь не пишется ни слова: лист и номер строки у
#: предпросмотра точные, и подставляет их экран из полей самой строки.
ISSUE_HELP: dict[str, str] = {
    "orphan_continuation": (
        "Выше по листу нет строки с артикулом и наименованием, от которой эта "
        "строка могла бы их унаследовать, — поэтому позиция у неё неизвестна. "
        "Разумное действие: открыть эту строку в источнике и посмотреть, к "
        "какой позиции она относится."),
    "identity_missing_part": (
        "Из двух частей имени позиции заполнена только одна: есть артикул без "
        "наименования либо наоборот. Вторую половину предпросмотр не "
        "угадывает. Разумное действие: посмотреть строку в источнике."),
    "quantity_missing": (
        "У позиции не прочитано ни одного количества: размеры XS–XL пусты и "
        "итог источника тоже. Разумное действие: проверить, в тех ли колонках "
        "записаны количества."),
    "invalid_quantity": (
        "В колонке количества написано не число — исходный текст показан "
        "рядом как есть. Ноль вместо него не подставляется: ноль означал бы "
        "«решили не шить», а это была бы выдумка."),
    "total_mismatch": (
        "Итог источника и сумма размеров этой строки не совпали. Оба числа "
        "показаны как написаны; предпросмотр не «чинит» одно другим. "
        "Разумное действие: сверить строку в источнике."),
    "unknown_column": (
        "В строке есть колонка, которой этот раздел не назначает смысла. Её "
        "текст сохранён целиком и показан в подробностях строки. Менять или "
        "убирать колонку в источнике из-за этого не нужно."),
}

#: Та же шестёрка ещё раз — коротко, для пометки в плотной строке таблицы.
#: Отдельный список нужен затем, чтобы подпись помещалась в одну строку и не
#: раздувала высоту: «Итог источника не сходится с суммой размеров» в ячейке
#: шириной в два слова переносится на три строки и делает высокой ВСЮ строку.
#: Полная подпись из `ISSUE_LABELS` и объяснение из `ISSUE_HELP` стоят рядом
#: в раскрываемых подробностях той же строки — короткая пометка ничего не
#: заменяет и ничего не скрывает.
ISSUE_SHORT: dict[str, str] = {
    "orphan_continuation": "нет строки-родителя",
    "identity_missing_part": "имя неполное",
    "quantity_missing": "нет количеств",
    "invalid_quantity": "не число",
    "total_mismatch": "итог ≠ сумма",
    "unknown_column": "чужая колонка",
}
...
def _counts_view(rows: list[dict], sheet_names: list[str]) -> dict:
    ...
    ЧТО ЭТА ФУНКЦИЯ ДОБАВЛЯЕТ СВЕРХ `build_counts` И ПОЧЕМУ ИМЕННО ЗДЕСЬ.
    `quantity_unknown_rows` — сколько строк набора не дали нам своего
    количества. Число не новое и не вычисляется по новому правилу: тот же
    предикат `_row_quantity_unknown` уже решает, окончателен ли агрегат
    (`quantity_complete`). Раньше наружу уходило только «полно/неполно», и
    экран мог сказать «прочитано не всё», но не мог сказать, чего именно не
    хватает; человек читает «593 шт» и вправе спросить, сколько строк за этой
    цифрой не стоит.

    Живёт оно ТОЛЬКО на чтении, а не в `build_counts`, и это не вкусовщина:
    `build_counts` пишется в носитель, и добавить туда поле значило бы
    изменить сохраняемый снимок ради подписи на экране. Читатель и так
    пересчитывает сводку из строк целиком, поэтому подпись выводится, а
    носитель не трогается ни на байт.
    """
    counts = build_counts(rows, sheet_names)

    def unknown(subset: list[dict]) -> int:
        return sum(1 for r in subset
                   if not r.get("is_blank") and _row_quantity_unknown(r))

    for block in counts["sheets"]:
        block["quantity_unknown_rows"] = unknown(
            [r for r in rows if r.get("sheet_name") == block["sheet_name"]])
    counts["quantity_unknown_rows"] = unknown(rows)
    return counts
...
def _load_config(conn: Connection) -> dict:
    ...
    try:
        ...
        # РАЗБОР ПАДАЕТ РАНЬШЕ ЛЮБОЙ НАШЕЙ ПРОВЕРКИ, и это не гипотетика.
        # Декодер `json` рекурсивен там, где нет C-ускорителя, — а он есть не
        # везде: на Python 3.11, где идёт CI проекта, достаточно вложенности
        # около двух тысяч, чтобы `json.loads()` бросил `RecursionError` прямо
        # здесь. Итеративный обход, который ниже ограничивает глубину, до этого
        # места просто не доживает.
        #
        # Я это уже один раз недооценил и говорю прямо: измерил на локальном
        # Python 3.14, где стоит C-сканер, увидел «не воспроизводится» и принял
        # это за ответ. Мерить надо было на том рантайме, который считается
        # авторитетным, — CI на нём и упал (`supply_sheets NO_REPORT`).
        #
        # `RecursionError` — не `ValueError` и не ловится веткой выше: он
        # наследуется от `RuntimeError`. Поэтому ветка отдельная, а исход тот
        # же, что у любой другой порчи носителя: управляемый 409, фиксированный
        # текст без единой строки чужих данных, и ни одной записи.
        raise CarrierConfigError(
            "настройки подключения вложены слишком глубоко, чтобы их прочитать "
            "— предпросмотр не стал их переписывать"
        ) from None
...
def _unrepresentable(where: str) -> CarrierConfigError:
    """Отказ по непредставимому тексту. Метка — ТОЛЬКО наш литерал.

    Ни ключа, ни значения в тексте нет и быть не может: и то и другое приехало
    из чужой таблицы или из чужой записи в носитель. По той же причине метка
    не собирается из пути вглубь — сегменты этого пути тоже бывают чужими.
    Называется верхний уровень, и этого достаточно, чтобы человек понял, где
    смотреть.
    """
    return CarrierConfigError(
        f"Сохранённый предпросмотр повреждён: в поле «{where}» есть текст, "
        f"который невозможно прочитать. Он оставлен как есть и не переписан."
    )
...
def _too_deep(where: str) -> CarrierConfigError:
    ...
    return CarrierConfigError(
        f"Сохранённый предпросмотр повреждён: в поле «{where}» вложенность "
        f"глубже допустимых {MAX_RESPONSE_DEPTH} уровней. Он оставлен как есть "
        f"и не переписан."
    )


def _reject_unrepresentable(value, where: str) -> None:
    """РЕКУРСИВНО: ни одной строки, которую нельзя отдать наружу.

    ЗАЧЕМ ОТДЕЛЬНЫЙ ОБХОД, ЕСЛИ ПОЛЯ УЖЕ ПРОВЕРЯЮТСЯ ПООТДЕЛЬНОСТИ. Потому что
    перечисление полей — это ровно тот способ, которым этот дефект пережил три
    корректива подряд. Сначала были закрыты имена листов, потом шесть
    контейнеров строки, и каждый раз оставалось то, что в перечислении не
    участвовало: незнакомые поля строки (они разрешены НАМЕРЕННО, ради
    совместимости с будущими версиями и с откатом), их вложенные списки и
    карты, ключи этих карт и половина верхнеуровневых полей снимка. Проверка,
    которая держится на полноте списка, ломается ровно тогда, когда список
    перестаёт быть полным, — а он перестаёт при первом же новом поле.

    Поэтому здесь не список, а СВОЙСТВО: ничто из того, что может уехать в
    ответ, не должно содержать строку, непредставимую в UTF-8. Обход идёт по
    словарям (и по КЛЮЧАМ тоже), спискам и кортежам на любую глубину, и
    незнакомые поля он не запрещает — он лишь требует, чтобы они были
    читаемыми. Совместимость с будущим сохраняется целиком: неизвестное поле
    с нормальным текстом проходит как прежде.

    Метка `where` фиксированная на всё поддерево и не собирается из пути:
    ...
    примерно на тысяче уровней, поэтому глубокий, но совершенно читаемый
    носитель ронял страницу без единого суррогата. Отсюда `MAX_RESPONSE_DEPTH`:
    отказ управляемый, а не 500. Нашёл это не ревью, а собственный тест этого
    же корректива — тот самый, что стоял замком на рекурсию.

    ОБХОД ИТЕРАТИВНЫЙ, И ЭТО НЕ СТИЛЬ. Первая редакция была рекурсивной, и она
    добавляла НОВЫЙ способ уронить страницу: глубина носителя нами не
    ограничена, а разбор её не ограничивает тем более — замерено на этом
    рантайме, `json.loads` и `json.dumps` спокойно проходят 20 000 уровней
    вложенности (C-сканер), тогда как предел рекурсии Python — 1000. То есть
    достаточно было положить в незнакомое поле строки глубокую вложенность,
    чтобы проверка, написанная ПРОТИВ 500, сама дала 500. Стек здесь явный, и
    глубина носителя на него не влияет.
    """
    stack = [(value, 0)]
    while stack:
        item, depth = stack.pop()
        if depth > MAX_RESPONSE_DEPTH:
            raise _too_deep(where)
...
def _validate_row(row: dict, index: int) -> None:
    ...
    # И ВСЯ строка целиком — включая поля, которых этот слой не знает. Они
    # разрешены намеренно (совместимость с будущими версиями и с откатом), и
    # запрещать их нельзя; но `preview()` копирует строку ЦЕЛИКОМ через
    # `dict(row)`, поэтому незнакомое поле уезжает в ответ наравне со знакомым.
    # Проверки выше дают точное имя поля там, где оно нам известно; эта —
    # закрывает всё остальное, не требуя знать имена заранее.
    _reject_unrepresentable(row, f"строка №{index + 1}")
...
def _validate_envelope(envelope: dict) -> dict:
    ...
    # Верхний уровень снимка целиком. `rows` пропускается: их проверяет
    # `_validate_row()` построчно и с номером строки в отказе.
    #
    # `last_attempt_source` пропускается НАМЕРЕННО и это единственное
    # исключение. Он читается только через `_attempt_identity()`, который
    # отбрасывает непредставимые части и по своему контракту обязан давать «не
    # исключение, а недоказуемую связь». Испорченный источник ПОПЫТКИ — не
    # повод объявить повреждённым весь снимок и спрятать от человека удачно
    # прочитанные строки: попытка не данные, а отметка о них. Что этот путь
    # действительно безопасен, проверяет не рассуждение, а сеть на границе
    # ответа ниже — она смотрит на то, что реально уезжает наружу.
    for key, value in envelope.items():
        if key in ("rows", "last_attempt_source"):
            continue
        label = key if key in _ENVELOPE_OWN_KEYS else "постороннее поле"
        if isinstance(key, str) and not _utf8_safe(key):
            raise _unrepresentable("постороннее поле")
        _reject_unrepresentable(value, label)
...
#: Ключи снимка, ИМЕНА которых принадлежат нам, — только они могут попасть в
#: текст отказа. Набор берётся из того, что пишет наш же писатель, чтобы он не
#: разошёлся со скелетом при следующей правке. Всё прочее в отказе называется
#: «постороннее поле»: имя незнакомого ключа — тоже чужой текст.
_ENVELOPE_OWN_KEYS = frozenset(_skeleton("", []).keys())
...
def refresh(db: Session, org_id: int, spreadsheet_url: str, sheet_names,
    ...
    with _org_lock(org_id):
        ...
        try:
            ...
            raise
        _store(db, org_id, envelope)
        # Та же сеть на втором выходе. Сработать она здесь не может: `_store()`
        # уже закодировал снимок в UTF-8 при проверке размера, а `decode_csv()`
        # читает источник строгим UTF-8 — из живого чтения непредставимая
        # строка не рождается вовсе. Сеть стоит не потому, что путь опасен, а
        # потому что «этот выход безопасен» — рассуждение, а рассуждения здесь
        # уже трижды оказывались неполными.
        return _safe_response({
            "unchanged": False,
            "content_sha256": digest,
            "fetched_at": now,
...
#: Поля строки, по которым идёт поиск. Ровно три и названы в интерфейсе:
#: наименование, артикул, цвет. Список закрыт намеренно — «искать везде»
#: означало бы искать и в сырых комментариях, где лежит свободный текст чужой
#: таблицы, и тогда человек не смог бы объяснить себе, почему нашлась строка,
#: в которой искомого слова не видно.
#:
#: Берутся `name`/`article`, а не `name_raw`/`article_raw`: у строки-продолжения
#: собственные ячейки идентичности пусты, а позиция у неё та же, что у якоря.
#: Человек, который ищет позицию, ищет все её строки, а не только первую.
SEARCH_FIELDS = ("name", "article", "color_raw")


def normalize_search(text: str) -> str:
    """Нормализация обеих сторон сравнения: регистр и внешние пробелы.

    Правило намеренно одно и объяснимое вслух. Ни синонимов, ни транслита, ни
    «умного» совпадения: поиск обязан быть предсказуемым, а не догадливым —
    иначе к «показано N из M» появляется вопрос, на который у человека нет
    ответа. `casefold()`, а не `lower()`: он единственный верно складывает
    регистр за пределами латиницы.
    """
    return text.strip().casefold()


def search_rows(rows: list[dict], query: str) -> list[dict]:
    """Строки, в которых искомое встречается в наименовании, артикуле или цвете.

    Пустой запрос не фильтрует ВООБЩЕ и не трогает набор: «ничего не искали» и
    «искали и не нашли» — разные состояния, и подменять первое вторым нельзя.

    Пустая строка-разделитель непустому запросу не соответствует никогда: все
    три её поля пусты. Это не отдельное правило, а следствие — но следствие
    важное, и поэтому названо: в результатах поиска разделители чужого листа
    не мешаются.
    """
    ...
    for row in rows:
        for field in SEARCH_FIELDS:
            value = row.get(field)
            if isinstance(value, str) and needle in normalize_search(value):
                out.append(row)
                break
    return out
...
def _safe_response(payload: dict, where: str) -> dict:
    """СЕТЬ НА ГРАНИЦЕ ОТВЕТА: наружу не уходит ничего непредставимого.

    Это не третья проверка того же — это единственная, которая не зависит от
    полноты чьего-либо списка. Проверки снимка знают, что искать; эта знает,
    ЧТО УХОДИТ. Добавят завтра в ответ новое поле, придёт оно из носителя, из
    каталога или откуда угодно ещё — сеть увидит его, потому что смотрит на
    готовый ответ, а не на перечень источников.

    Именно перечисления и подвели этот слой трижды подряд: закрывали имена
    листов, потом шесть контейнеров строки, и каждый раз наружу оставался путь,
    которого в перечне не было. Поэтому здесь инвариант формулируется один раз
    и на результат: `JSONResponse` обязан суметь закодировать то, что мы ему
    отдаём, — иначе человек получит 500 вместо экрана.

    Отказ тот же, что и у остальной порчи носителя: `CarrierConfigError` → 409,
    без единой записи и без эха значения. Стоимость — один проход по уже
    собранному ответу; он ограничен теми же пределами, что и сам снимок.
    """
    _reject_unrepresentable(payload, where)
    return payload


def preview(db: Session, org_id: int, role: str, sheet: str | None = None,
            queue: str = "all", offset: int = 0, limit: int = 50,
            q: str = "") -> dict:
    """Страница снимка для GET. Только чтение, ни одной мутации."""
    if queue not in QUEUES:
        raise ValidationError("Неизвестный фильтр очереди.")
    # Поисковый запрос — чужой ввод, и проверяется он как чужой ввод: 400, а не
    # 409. Непредставимую строку ловим здесь и по имени, потому что сеть на
    # границе ответа (`_safe_response`) назвала бы её порчей носителя — то есть
    # ответила бы «база испорчена» на то, что человек только что прислал сам.
    if q is None:
        q = ""
    if not isinstance(q, str):
        raise ValidationError("Поисковый запрос должен быть текстом.")
    if not _utf8_safe(q):
        raise ValidationError("В поисковом запросе есть символы, которые "
                              "невозможно прочитать.")
    if len(q) > MAX_SEARCH_CHARS:
        raise ValidationError(
            f"Поисковый запрос длиннее {MAX_SEARCH_CHARS} символов.")
    try:
        offset = int(offset)
        limit = int(limit)
    ...
    base = {
        ...
        "queue": queue,
        "sheet": sheet or "",
        # Запрос возвращается тем же, каким пришёл: экран сверяет с ним своё
        # поле ввода, и «показано N из M» обязано относиться к тому запросу,
        # который человек видит в поле, а не к предыдущему.
        "q": q,
        "offset": offset,
        "limit": limit,
        "issue_labels": dict(ISSUE_LABELS),
        "issue_help": dict(ISSUE_HELP),
        "issue_short": dict(ISSUE_SHORT),
    ...
    if not envelope:
        ...
        return _safe_response(base, "ответ предпросмотра")
    ...
    # ОТБОР ИДЁТ ПО ВСЕМУ ПРОВЕРЕННОМУ НАБОРУ И ДО СТРАНИЦЫ. Лист, очередь и
    # поиск сужают снимок целиком, и только после этого из результата берётся
    # окно `offset:offset+limit`. Поэтому `total` — это размер применимого
    # набора, а не число загруженных строк, и догрузка идёт по нему же.
    # Обратный порядок (искать внутри загруженных 50) дал бы счётчик, который
    # меняется от того, сколько человек успел долистать, — то есть число,
    # которому нельзя верить.
    selected = search_rows(filter_rows(rows, sheet or None, queue), q)
