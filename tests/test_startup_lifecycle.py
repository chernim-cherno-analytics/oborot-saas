...
проверка ловила бы ложный успех: старый вызов уже случился бы мимо счётчика.
  ...
  1) реальный порядок: все пятнадцать шагов старта (init_db,
...
руками собранная прежняя схема), без боевых данных:
  ...
  5) чистая база: журнал содержит ровно пятнадцать объявленных шагов, позиции
  ...
  7) повторный старт идемпотентен: строк по-прежнему пятнадцать, applied_at
     первой записи НЕ переписан, и при этом все пятнадцать шагов выполнились
...
STEPS = [
    ...
    # SUPPLY-FIX-4 (F-23б): шестой append-only шаг. Позиция 15 новая,
    # четырнадцать прежних пар не тронуты — четырнадцатая тем более, она уже
    # выпущена на прод. Шаг добавляет одну нуллируемую колонку BLOB и ни одной
    # строки не читает.
...
# Тринадцать шагов до отметки архива у назначения. Список опять наращивается
...
# Четырнадцать шагов до миниатюры эскиза. Список опять наращивается явным
# сложением, а не срезом, и по той же причине: прежние строки журнала боевой
# базы не двигаются.
RELEASED_BEFORE_THUMB = RELEASED_BEFORE_ASSIGN_ARCHIVE + [ASSIGN_ARCHIVE_STEP]
THUMB_STEP = ("models.ensure_supply_sketch_thumb_schema", 15)

# Общий пролог дочернего процесса: подменяет пятнадцать шагов старта и
...
_orig_aarch = _models.ensure_supply_assignment_archive_schema
def _w_aarch(*a, **kw):
...
def check_order() -> None:
    """Проверка 1: реальный порядок пятнадцати шагов старта и scheduler.start."""
...
"""
    ...
    check("зафиксированы все пятнадцать шагов старта",
...
def check_ledger_clean_db() -> None:
    """Проверка 5: на чистой базе журнал содержит ровно пятнадцать шагов."""
...
def check_ledger_legacy_db() -> None:
    ...
    check("все пятнадцать шагов выполнились и на базе без журнала",
...
def check_ledger_repeat_startup() -> None:
    ...
    check("повторный старт ВЫПОЛНИЛ все пятнадцать шагов (журнал не повод пропускать)",
...
def check_ledger_supply_step_is_distinct() -> None:
    ...
    # SUPPLY-FIX-2 (13 и 14) и SUPPLY-FIX-4 (15). Их ровно шесть, они идут
    ...
    check("новые строки — SUPPLY-1 (10), SUPPLY-3 (11), SUPPLY-FIX-1 (12), "
          "SUPPLY-FIX-2 (13 и 14), SUPPLY-FIX-4 (15)",
          ...
          and (fresh[5][0], fresh[5][1]) == THUMB_STEP, f"new={fresh}")
    ...
    check("повторный старт журнал не изменил: те же пятнадцать строк и то же время",
