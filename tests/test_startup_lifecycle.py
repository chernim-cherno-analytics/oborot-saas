...
проверка ловила бы ложный успех: старый вызов уже случился бы мимо счётчика.
  ...
  1) реальный порядок: все тринадцать шагов старта (init_db,
...
руками собранная прежняя схема), без боевых данных:
  ...
  5) чистая база: журнал содержит ровно тринадцать объявленных шагов, позиции
  ...
  7) повторный старт идемпотентен: строк по-прежнему тринадцать, applied_at
     первой записи НЕ переписан, и при этом все тринадцать шагов выполнились
...
STEPS = [
    ...
    # SUPPLY-FIX-1: тот же append-only контракт в третий раз. Позиция 12 новая,
    # одиннадцать прежних пар не тронуты. Этот шаг, в отличие от двух
    # предыдущих, ещё и ТРОГАЕТ введённые строки — схлопывает дубли перед тем,
    # как поставить уникальные индексы.
    "models.ensure_supply_planning_unique_schema",
    # SUPPLY-FIX-2: четвёртый раз. Позиция 13 новая, двенадцать прежних пар не
    # тронуты — и двенадцатая тем более, она уже выпущена на прод. Шаг самый
    # узкий из четырёх: три ALTER ... ADD COLUMN и ни одной прочитанной строки.
    "models.ensure_supply_archive_schema",
]
...
# Одиннадцать шагов, ВЫПУЩЕННЫХ до SUPPLY-FIX-1. Список опять наращивается
# явным сложением, а не срезом: смысл в том, что прежние строки журнала боевой
# базы не двигаются.
RELEASED_BEFORE_UNIQUE = RELEASED_BEFORE_PLANNING + [PLANNING_STEP]
UNIQUE_STEP = ("models.ensure_supply_planning_unique_schema", 12)

# Двенадцать шагов, ВЫПУЩЕННЫХ до SUPPLY-FIX-2 (шаг 12 выпущен на прод
# 06.09.2026, журнал выпуска 5559927093). Список снова наращивается явным
# сложением, а не срезом: смысл ровно в том, что прежние строки не двигаются.
RELEASED_BEFORE_ARCHIVE = RELEASED_BEFORE_UNIQUE + [UNIQUE_STEP]
ARCHIVE_STEP = ("models.ensure_supply_archive_schema", 13)

# Общий пролог дочернего процесса: подменяет тринадцать шагов старта и
...
_orig_unique = _models.ensure_supply_planning_unique_schema
def _w_unique(*a, **kw):
    order.append("models.ensure_supply_planning_unique_schema")
    return _orig_unique(*a, **kw)
_models.ensure_supply_planning_unique_schema = _w_unique
_orig_archive = _models.ensure_supply_archive_schema
def _w_archive(*a, **kw):
    order.append("models.ensure_supply_archive_schema")
    return _orig_archive(*a, **kw)
_models.ensure_supply_archive_schema = _w_archive
"""
...
def check_order() -> None:
    """Проверка 1: реальный порядок тринадцати шагов старта и scheduler.start."""
...
"""
    ...
    check("зафиксированы все тринадцать шагов старта",
...
def check_failure_prevents_scheduler_start() -> None:
    """Проверка 2: сбой на КАЖДОМ шаге старта — планировщик не стартует,
...
def check_ledger_clean_db() -> None:
    """Проверка 5: на чистой базе журнал содержит ровно тринадцать шагов."""
    ...
    check("журнал содержит ровно тринадцать строк", len(rows) == 13, f"rows={rows}")
    check("id и позиции журнала совпадают с объявленным порядком",
          [(r[0], r[1]) for r in rows] == LEDGER_STEPS, f"rows={rows}")
    check("позиции идут 1..13 по возрастанию без пропусков",
          [r[1] for r in rows] == list(range(1, 14)), f"rows={rows}")
...
def check_ledger_legacy_db() -> None:
    ...
    check("все тринадцать шагов выполнились и на базе без журнала",
...
def check_ledger_repeat_startup() -> None:
    ...
    check("повторный старт не добавил строк в журнал", len(second) == 13, f"rows={second}")
    ...
    check("повторный старт ВЫПОЛНИЛ все тринадцать шагов (журнал не повод пропускать)",
...
def check_ledger_not_recorded_on_failure() -> None:
    ...
    # Последний в списке — терминальный шаг: он проверяется отдельно и
...
def check_ledger_supply_step_is_distinct() -> None:
    ...
    check("старт на базе с девятью выпущенными шагами не упал",
          "SCHEDULER_START_CALLED True" in out, out[-300:])
    rows = _read_ledger(db)
    # На базе с девятью выпущенными шагами дописываются ЧЕТЫРЕ терминальных:
    # SUPPLY-1 (позиция 10), SUPPLY-3 (11), SUPPLY-FIX-1 (12) и SUPPLY-FIX-2
    # (13). Их ровно четыре, они идут своим порядком, и у каждого своё время.
    check("журнал прирос ровно четырьмя строками", len(rows) == 13, f"rows={rows}")
    ...
    check("новые строки — SUPPLY-1 (10), SUPPLY-3 (11), SUPPLY-FIX-1 (12), "
          "SUPPLY-FIX-2 (13)",
          len(fresh) == 4 and (fresh[0][0], fresh[0][1]) == SUPPLY_STEP
          and (fresh[1][0], fresh[1][1]) == PLANNING_STEP
          and (fresh[2][0], fresh[2][1]) == UNIQUE_STEP
          and (fresh[3][0], fresh[3][1]) == ARCHIVE_STEP, f"new={fresh}")
    check("у всех четырёх собственный applied_at, а не время выпущенных шагов",
          len(fresh) == 4 and all(f[2].endswith("Z") and f[2] != seeded[0][2]
    ...
    check("повторный старт журнал не изменил: те же тринадцать строк и то же время",
