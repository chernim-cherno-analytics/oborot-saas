...
def run_scenario() -> int:
    ...
    #
    # ВХОД ЗАДАЁТСЯ ЦЕЛИКОМ, А НЕ ОДНИМ РАЗМЕРОМ. Прежняя редакция посылала
    # `{first_size: 1}`, и остальные размеры оставались расчётными — значит
    # `expect_need` зависел от рекомендации ДНЯ. В сутки, когда сумма случайно
    # проходила и минимальную партию, и кратность, условие ниже (:3122) было
    # ложным, проверка предупреждения не исполнялась вовсе, и сторож полноты
    # шарда видел 330 проверок вместо 331. Один и тот же код давал то зелёный,
    # то красный обязательный CI в зависимости от календаря.
    #
    # Теперь все размеры названы явно: один в 1, остальные в 0. Итог заведомо
    # равен 1 — он нарушает и минимум 30, и кратность 6 при ЛЮБОЙ рекомендации,
    # поэтому условная ветка исполняется всегда. Эталон 331 не тронут, ни одна
    # проверка не удалена и не добавлена, порядок сохранён: чинится вход, а не
    # мерка.
    b_sizes = {k: v["rec"] for k, v in api_items[b_name]["sizes"].items()}
    first_size = sorted(b_sizes)[0]
    manual = {k: (1 if k == first_size else 0) for k in b_sizes}
