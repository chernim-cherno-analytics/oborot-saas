...
def run() -> int:
    ...
    # архив и возврат материала, правку, архив и возврат вещи). Сторож здесь не
    # про длину списка, а про то, что выборка вообще что-то нашла: без этой
    # строки пустой `posts` сделал бы следующую проверку зелёной ни на чём.
    check("пишущих маршрутов найдено столько, сколько их есть", len(posts) == 15,
          f"найдено: {len(posts)}")
