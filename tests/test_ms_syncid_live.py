...
Что проверяется:
  ...
  2) точный поиск по нашему syncId находит РОВНО ЕГО;
...
── Чем этот файл отличается от прежней редакции ─────────────────────────────

Прежняя редакция искала по `filter=syncId`. 25.08.2026 живой аккаунт ответил
на самый первый read-only preflight **HTTP 412, code 1034, «неизвестное поле
фильтрации syncId»** (Issue #2, issuecomment-5414290329) — и остановился
fail-closed, ничего не создав.
...
обещанием документа и наблюдаемым ответом здесь выбран ответ — и это общее
правило, а не разовая уступка.

Поэтому и продукт, и этот тест ищут одним и тем же способом
(`MoySkladClient.find_by_sync_id`), не зависящим от фильтра. Отдельного,
«тестового» способа поиска здесь нет намеренно: иначе тест доказывал бы
работу механизма, которого в продукте не существует.

── Две стадии, два разных разрешения ────────────────────────────────────────

Запуск разрезан надвое, и это прямое следствие 25.08:

  * **только чтение** (по умолчанию) — разведка контракта и все подготовительные
    проверки. Ни одной изменяющей операции;
  * **создание** — включается ОТДЕЛЬНОЙ переменной `OBOROT_LIVE_MS_CREATE` с
    её собственной дословной фразой, отличной от фразы запуска.

Один прогон не должен уметь и «посмотреть, что там», и завести документы:
иначе разведка рано или поздно окажется мутирующей. В тот раз до создания дело
не дошло по счастливой случайности — теперь это правило.
...
Стадия «только чтение» — ничего не создаёт:
...
Стадия создания — ОТДЕЛЬНЫМ запуском и только после того, как читающая
половина прошла:

    OBOROT_LIVE_MS_TOKEN=<токен аккаунта> \\
    OBOROT_LIVE_MS_CONFIRM=я-понимаю-что-создам-документы \\
    OBOROT_LIVE_MS_CREATE=создать-тестовые-сущности \\
        python tests/test_ms_syncid_live.py
...
# Вторая, ОТДЕЛЬНАЯ фраза — только для создающей половины прогона.
#
# Первый запуск обязан доказывать читающую половину контракта и на этом
# останавливаться: 25.08.2026 живой аккаунт отверг самый первый read-only
# preflight, и хорошо, что до создания дело не дошло. Один запуск не должен
# уметь и «посмотреть, что там», и завести документы — иначе разведка рано или
# поздно окажется мутирующей.
#
# Фраза намеренно ДРУГАЯ: если бы она совпадала с CONFIRM_PHRASE, разделения
# бы не было вовсе.
CREATE_PHRASE = "создать-тестовые-сущности"
CREATE = os.environ.get("OBOROT_LIVE_MS_CREATE", "")
...
def may_create() -> bool:
    """Разрешено ли ЭТОМУ запуску создавать сущности.

    Читается из модульной переменной, а не из окружения напрямую, чтобы
    проверки могли подставить значение и не зависеть от того, как запущен
    набор. Сравнение — дословное, по той же причине, что и у CONFIRM_PHRASE:
    «непустое значение» разрешением не является.
    """
    return CREATE.strip() == CREATE_PHRASE
...
async def _counterparties_by_sync_id(client, sync_id: str) -> list:
    ...
    Идёт тем же путём, что и продукт (`find_by_sync_id`), и по той же причине:
    `filter=syncId` живой аккаунт не принимает. Собственного, «тестового»
    способа поиска здесь нет намеренно — иначе тест доказывал бы работу
    механизма, которого в продукте не существует.
    """
    return await client.find_by_sync_id("counterparty", sync_id)


async def probe_read_only(client) -> None:
    """Read-only разведка контракта. Ни одной изменяющей операции.

    Здесь фиксируются ФАКТЫ о чужом API, а не проверяются наши ожидания.
    Ни один из этих ответов не является условием продолжения: продукт не
    зависит ни от фильтра, ни от точечного маршрута. Мы просто перестаём
    гадать и записываем, что аккаунт умеет на самом деле.
    """
    say("\n== P. Разведка контракта (только чтение) ==")
    probe_sync = str(uuid.uuid4())

    # 1) filter=syncId. 25.08.2026 живой аккаунт ответил 412/1034. Проверяем
    #    заново и записываем факт: если МойСклад однажды фильтр включит, мы
    #    узнаем об этом отсюда, а не из чужого предположения.
    try:
        rows = [r async for r in client.paginate(
            "/entity/counterparty", {"filter": f"syncId={probe_sync}"})]
        check("filter=syncId ПРИНЯТ этим аккаунтом (наблюдение, не требование)",
              True, f"строк={len(rows)}")
    except Exception as exc:  # noqa: BLE001 — это разведка, падать тут нечему
        resp = getattr(exc, "response", None)
        code = getattr(resp, "status_code", "?")
        check("filter=syncId ОТВЕРГНУТ этим аккаунтом "
              "(ожидаемо: продукт на него не опирается)",
              True, f"{type(exc).__name__} HTTP {code} {scrub(exc)[:160]}")

    # 2) точечный маршрут /entity/{type}/syncid/{id}. Документация описывает
    #    его только для удаления; работает ли GET — неизвестно. Отрицательный
    #    ответ здесь ничего не значит и ни на что не влияет.
    point = await client.sync_id_point_lookup("counterparty", probe_sync)
    check("точечный syncid-запрос по свободному ключу ничего не подтвердил "
          "(так и должно быть — ключ свободен)", point is None,
          f"ответ={point and point.get('id')}")

    # 3) то, на чём продукт стоит на самом деле: точный поиск нашим способом.
    #    Свободный ключ обязан находиться нулём раз.
    found = await client.find_by_sync_id("counterparty", probe_sync)
    check("ТОЧНЫЙ ПОИСК НАШИМ СПОСОБОМ РАБОТАЕТ и по свободному ключу "
          "возвращает пусто", found == [], f"найдено={len(found)}")
...
async def run_contract(client) -> int:
    ...
    say(f"Стадия: {'СОЗДАНИЕ' if may_create() else 'ТОЛЬКО ЧТЕНИЕ'}")
    try:
        await probe_read_only(client)
        pre = await prerequisites(client, tag, agent_sync, doc_sync)
        if not may_create():
            # Первый запуск доказывает ЧИТАЮЩУЮ половину контракта и на этом
            # останавливается. Разрешение на создание — отдельная переменная и
            # отдельный запуск: один прогон не должен уметь и проверить, и
            # создать, иначе «посмотреть, что там» однажды заведёт документы.
            say("\nСоздание сущностей НЕ разрешено этим запуском — остановка "
                "после read-only проверок. Чтобы выполнить создающую часть, "
                f"задайте OBOROT_LIVE_MS_CREATE={CREATE_PHRASE!r} отдельным "
                "запуском.")
            report_ledger(tag)
