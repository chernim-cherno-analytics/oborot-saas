...
Коды возврата — часть контракта этого файла, а не деталь реализации:

    0 — контракт доказан ЦЕЛИКОМ: обе создающие стадии дошли до конца и ни
        одна проверка не провалилась. Только этот код закрывает гейт;
    1 — проверка провалилась (на любой стадии);
    2 — запуск не разрешён: нет токена либо фраза подтверждения не та;
    3 — прогон дошёл до конца ЧИТАЮЩЕЙ половины и честно остановился, либо
        оборвался, не завершив обязательные стадии. Контракт НЕ доказан.

Ноль возвращается не потому, что «список провалов пуст», а потому, что обе
обязательные стадии отмечены завершёнными: иначе любой будущий выход из
сценария, не успевший записать провал, выглядел бы успехом.

Читающая стадия строку «ИТОГО: N OK, 0 FAIL» не печатает вовсе (ревью Codex,
P1, discussion_r3858423224). Раньше печатала — и её вывод был неотличим от
доказанного контракта: достаточно было один раз запустить разведку, чтобы в
журнале выпуска появилось «живой контракт пройден», хотя ни один обязательный
POST/upsert/пересчёт уникальности не выполнялся.
...
Проверки самого этого файла, не требующие сети, живут в блоках 15–18 и 28
`tests/test_writeback_idempotency.py`: точность фразы, разбор payload,
доказательство, что после первой ошибки следующая мутирующая стадия не
вызывается, и приговоры с кодами возврата.
"""
import asyncio
import os
import sys
...
# ── Приговор прогона: код возврата — часть контракта, а не деталь ───────────
#
# Ревью Codex, P1 (discussion_r3858423224). Читающая стадия возвращала 0 и
# печатала «ИТОГО: N OK, 0 FAIL» — ровно то же, что полностью доказанный
# контракт. Для автоматики и журнала выпуска это неотличимо, и достаточно было
# один раз запустить разведку, чтобы в журнале появилось «живой контракт
# пройден», хотя ни одного обязательного POST/upsert/уникальности не
# выполнялось. Гейт слияния закрывался бы на бумаге, оставаясь открытым.
#
# Поэтому приговоров четыре, и они означают разное:
EXIT_PASS = 0           # контракт доказан ЦЕЛИКОМ: обе создающие стадии дошли
                        # до конца и ни одна проверка не провалилась;
EXIT_FAIL = 1           # проверка провалилась — на любой стадии;
EXIT_NOT_AUTHORIZED = 2 # запуск не разрешён (нет токена или неверная фраза);
EXIT_NOT_COMPLETE = 3   # прогон честно дошёл до конца читающей половины и
                        # остановился: контракт НЕ доказан, гейт НЕ закрыт.

# Обязательные стадии полного контракта. `EXIT_PASS` требует, чтобы КАЖДАЯ
# была отмечена завершённой — иначе «ноль» означал бы всего лишь «список FAIL
# пуст». Разница не теоретическая: любой выход из сценария, не успевший
# записать FAIL, давал бы успех. Отметка ставится ПОСЛЕ возврата стадии,
# то есть после всех её проверок, включая повторный POST и пересчёт
# уникальности перебором.
STAGE_COUNTERPARTY = "контрагент"
STAGE_ORDER = "заказ поставщику"
REQUIRED_STAGES = (STAGE_COUNTERPARTY, STAGE_ORDER)
...
def verdict(*, read_only: bool) -> int:
    """Печатает приговор прогона и возвращает код возврата.

    Одно место на оба пути намеренно: пока «итого» печаталось в двух местах
    одинаковой строкой, читающая половина и доказанный контракт выглядели
    одинаково — с этого и начался finding.

    Форма `ИТОГО: N OK, M FAIL` осталась ТОЛЬКО у терминальных приговоров.
    У незавершённого прогона её нет вовсе, поэтому «0 FAIL» у частичного
    запуска не может напечататься даже случайно: строки просто не существует.
    """
    if FAIL:
        say(f"\nИТОГО: {len(PASS)} OK, {len(FAIL)} FAIL")
        say("ВЕРДИКТ: ПРОВАЛЕН — живой контракт syncId НЕ подтверждён.")
        return EXIT_FAIL
    if read_only:
        say(f"\nПроверок пройдено: {len(PASS)}; провалов нет. "
            "Но это ПОЛОВИНА контракта.")
        say("ВЕРДИКТ: НЕ ВЫПОЛНЕН — доказана только читающая часть. "
            "Обязательные проверки DATA-1 (POST с нашим syncId, повторный "
            "POST с тем же id, ровно один объект по ключу после повтора) "
            "НЕ выполнялись.")
        say(f"ГЕЙТ СЛИЯНИЯ DATA-1/DATA-2 НЕ ЗАКРЫТ. Код возврата "
            f"{EXIT_NOT_COMPLETE}.")
        return EXIT_NOT_COMPLETE
    missing = [st for st in REQUIRED_STAGES if st not in STAGES_DONE]
    if missing:
        # Сценарий оборвался, но ни одна проверка не записала FAIL. Это не
        # успех: обязательные утверждения не выполнялись, а значит контракт
        # не доказан. Раньше такой выход давал бы ровно ноль.
        say(f"\nПроверок пройдено: {len(PASS)}; провалов нет, но сценарий "
            f"не дошёл до конца: не завершено — {', '.join(missing)}.")
        say("ВЕРДИКТ: НЕ ВЫПОЛНЕН — контракт не доказан целиком.")
        say(f"ГЕЙТ СЛИЯНИЯ DATA-1/DATA-2 НЕ ЗАКРЫТ. Код возврата "
            f"{EXIT_NOT_COMPLETE}.")
        return EXIT_NOT_COMPLETE
    say(f"\nИТОГО: {len(PASS)} OK, {len(FAIL)} FAIL")
    say("ВЕРДИКТ: ПРОЙДЕН — живой контракт syncId подтверждён целиком "
...
async def run_contract(client) -> int:
    ...
    return verdict(read_only=False)
