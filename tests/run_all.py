...
SUITES = [
    ...
    # SUPPLY-2, часть в браузере. Три замечания ревью PR #47 живут целиком на
    # стороне страницы (read-only состояние подписки, сериализованная догрузка,
    # сброс устаревшего фильтра листа), и проверкой «строка есть в шаблоне» ни
    # одно из них не ловится. Набор браузерный, как `ui` и `onbprev`, и стоит
    ...
    # SUPPLY-COMPAT: пишущие маршруты плана против ЧУЖОГО замка схемы. Набор
    # стоит отдельно от `supply_planning`, а не внутри него, по существу: тот
    # проверяет слой на СВОЕЙ схеме, а этот — на схеме, которой в выпущенной
    # версии ещё нет и которая приходит откатом сверху. Индексы он ставит сам,
    # дословным DDL будущего шага старта; схему проекта пакет не меняет.
    # Офлайновый, как и соседи: ни мока МойСклада, ни телеграма, ни сети.
    ("supply_compat", "test_supply_compat.py", False, False),
    ...
    # PILOT-RECEIPTS-UI-1: экран приёмки заказа (частичный приход, довоз,
    # исправление минусом, история). Набор браузерный, как `ui`, и стоит рядом
    # с ним. Мока МойСклада и телеграма ему не нужно: сервер локальный, данные
    # синтетические, каталог из демо-сида проекта, наружу набор не ходит.
    # Ручки приёмки на уровне API проверяет `exec` — здесь проверяется то, что
    # видит и нажимает человек, и прежде всего различие «не знаем» против
    # «не приехало», которое проверкой разметки не ловится.
    ("receipts_ui", "test_receipts_ui.py",     False, False),
