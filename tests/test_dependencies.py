...
ПРИГОВОР (D-42). Набор заканчивается каноническим «ИТОГО: N OK, M FAIL» —
и заканчивается им ВСЕГДА, в том числе на раннем отказе. Прежний финал
«OK: N   FAIL: M» раннер не читал вовсе и выносил NO_REPORT: двенадцать
выполненных проверок засчитывались как «набора не было».

Отдельно про ранний отказ. Нет `requirements.lock` — остальные проверки
действительно бессмысленны, но это НЕ повод для пропуска с кодом 77. Пропуск
означает «проверить объективно нечем» (нет инструмента в машине); здесь же
отсутствует сам предмет проверки, который обязан лежать в репозитории, — то
есть ровно та поломка, ради которой набор и написан. Поэтому ранний отказ
даёт канонически посчитанный ПРОВАЛ («ИТОГО: 0 OK, 1 FAIL», код 1), а не
пропуск и не NO_REPORT. Прежде здесь печаталось «ИТОГО: без lock-файла
остальное проверять нечего.» — слово «ИТОГО» есть, чисел нет, приговор
NO_REPORT: набор молча выпадал из отчёта именно в том случае, ради которого
существует.

Кода 77 у этого набора нет ни на одной ветке: всё, что ему нужно, — это
Python и файлы репозитория, и если их нет, проверять нечего не потому, что
машина бедная, а потому что репозиторий сломан.
...
def report() -> int:
    """Канонический финал набора (D-42) и код возврата по нему.

    Единственное место, где набор подводит итог. Вынесено в функцию не для
    красоты: раньше финал печатался только в конце `main()`, и ранний `return`
    уходил мимо него — набор заканчивался без отчёта, который раннер умеет
    прочитать.
    """
    print("\n" + "=" * 62)
...
def main() -> int:
    ...
    if not LOCK.is_file():
        # Дальше проверять действительно нечего — но это ПРОВАЛ, а не пропуск:
        # lock-файл обязан лежать в репозитории, его отсутствие и есть та
        # поломка, ради которой набор написан. Проверка выше уже отработала и
        # уже в FAIL, поэтому отчёт честный и непустой: «ИТОГО: 0 OK, 1 FAIL».
        print("\nБез lock-файла остальные проверки бессмысленны — набор "
