...
# +2 (30.08.2026, календарная дыра в проверке пульса): «среднее продаж»/
# «средний склад» безусловно требовали хотя бы один полный покрытый месяц
# (bool(known)) и стабильно падали на коротких историях теста, где таких
# месяцев ноль (avg6=0 known=[]) — не регрессия рантайма, а дыра в самой
# проверке. Каждая из двух проверок в акте «core» разбита на явную проверку
# длины (совпадает ли известное с ожидаемым окном пульса, включая нулевой
# случай) и переименованную проверку среднего/честного нуля — набор имён
# проверок теперь не зависит от календаря, меняется только вердикт внутри.
LEGACY_TOTAL = 416
