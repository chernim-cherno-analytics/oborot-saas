...
Сценарий:
  ...
  4) тип есть в ассортименте, но не проставлен на одной карточке → синк
     проходит: это нормальное состояние каталога, а не авария;
  5) настройки пусты → синк проходит, эвристики работают как раньше;
  6) тип вернулся → синк снова done, и цены ОБНОВЛЯЮТСЯ.
