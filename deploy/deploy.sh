...
#
# Одно место здесь принципиально: определение «сломано» нужно и починке, и
# проверке. Разъедься они в нём — проверка начала бы зеленеть ровно на том,
# чего починка не трогала, а это тот же класс дефекта, который OPS-6 и закрывает.
#
...
# только у тех обёрток, чей интерпретатор ПЕРЕСТАЛ СУЩЕСТВОВАТЬ. Рабочий шебанг
# не трогается: починка, которая правит исправное, сама становится источником
# изменений, а окружение релиза обязано остаться ровно тем, что собрано по
# requirements.lock. Аргументы шебанга (`-sE` и подобное) сохраняются.
#
...
repair_console_shebangs() {
  ...
  [ "$fixed" = 0 ] || echo "   консольных обёрток починено после переезда: $fixed"
  return 0
}

# Проверка обёрток — независимая от починки и fail-closed. Смотрит на результат,
# а не на то, что починка о себе сообщила: сначала в $VENV/bin не должно
# остаться ни одной обёртки с несуществующим интерпретатором, а затем
...
verify_console_entrypoints() {
  ...
  if [ -n "$bad" ]; then
    echo "   консольные обёртки указывают на несуществующий интерпретатор:$bad" >&2
    return 1
  fi
