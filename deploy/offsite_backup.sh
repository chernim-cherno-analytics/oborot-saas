...
#
# Второе уточнение, по повторному ревью 23.08: «в хранилище есть снимок» — тоже
# не то условие. Оно выполняется вчерашней копией, если сегодняшняя загрузка
# вернула 0 и ничего не создала. Проверяется появление ИМЕННО НОВОГО снимка:
...
# Идентификатор последнего снимка нашего тега и хоста. Пустой вывод и КОД 0 —
# это «снимков нет», отдельный от «спросить не удалось» (код 1). Различать их
# обязательно: на этом различии стоит решение, удалять старые копии или нет.
latest_snapshot_id() {
  local json
  json="$("$RESTIC" snapshots --json --latest 1 --tag oborot-db --host "$HOST_TAG" 2>/dev/null)" \
    || return 1
  # Без `head -1` намеренно: под `pipefail` досрочно закрытый им конвейер даёт
  # 141 (SIGPIPE), и «спросить не удалось» появлялось бы на ровном месте. После
  # `tr -d` вход — одна строка, и `sed` выдаёт не больше одного совпадения.
  printf '%s' "$json" | tr -d ' \n\r\t' \
    | sed -n 's/.*"id":"\([0-9a-fA-F]\{6,\}\)".*/\1/p'
}

for c in sqlite3 flock mktemp find sed tr; do
...
# Снимок, который был последним ДО этой загрузки. Нужен, чтобы после неё
# доказать появление ИМЕННО НОВОГО снимка, а не просто «какой-то снимок в
# хранилище есть». Отказ здесь — до отправки: если хранилище отвечает на
# `cat config`, но не отдаёт список снимков, его метаданные нечитаемы, и
# загружать в него копию, которую потом нечем проверить, смысла нет.
BEFORE_ID="$(latest_snapshot_id)" \
  || die "хранилище отвечает, но список снимков получить не удалось.
   Без него нельзя доказать, что новая копия действительно появилась, — а без
   такого доказательства ротация удаляла бы старые копии вслепую.
   Ничего не загружено, старые копии целы."
...
# Порядок здесь и есть суть шага. Сначала убеждаемся, что появился НОВЫЙ снимок
# и что хранилище цело, — и только потом срезаем старое. Обратный порядок
# означает: повреждение нашли уже после того, как своими руками удалили копии,
# которыми его можно было бы пережить.
#
...
#
# И этого мало, что найдено внешним ревью 23.08. «Список не пуст» выполняется и
# тогда, когда в хранилище лежит ТОЛЬКО СТАРЫЙ снимок, а сегодняшняя загрузка
# соврала об успехе и ничего не создала: проверка проходит на вчерашней копии, и
# ротация срезает копии позавчерашние. День за днём это оставляет ровно один
# устаревающий снимок при зелёном отчёте. Поэтому сравнивается идентификатор:
# у restic каждый снимок получает новый id (в хеш входит время), даже если
# данные не изменились ни на байт.
AFTER_ID="$(latest_snapshot_id)" \
  || die "не удалось получить список снимков. Ротация НЕ выполнялась, старые копии целы."
[ -n "$AFTER_ID" ] || die "новый снимок в хранилище не виден: restic snapshots вернул пустой список.
   Загрузка отчиталась об успехе, а снимка нет. Ротация НЕ выполнялась,
   старые копии целы."
[ "$AFTER_ID" != "$BEFORE_ID" ] || die "НОВОГО снимка в хранилище не появилось.
   Последний снимок тот же, что и до загрузки ($AFTER_ID): restic backup
   отчитался об успехе, но ничего не создал. Ротация НЕ выполнялась, старые
   копии целы — иначе они были бы срезаны ради вчерашней копии."
"$RESTIC" check \
...
# В отметке остаётся id снимка: по нему видно, что каждую ночь появлялся НОВЫЙ
# снимок, а не проверялся один и тот же.
printf '%s host=%s orgs=%s snapshot=%s\n' "$STAMP" "$HOST_TAG" "$ORGS" "$AFTER_ID" \
  > "$STATE_DIR/last-offsite-backup.tmp"
mv "$STATE_DIR/last-offsite-backup.tmp" "$STATE_DIR/last-offsite-backup"
echo "ОФСАЙТ-БЭКАП ГОТОВ: $STAMP, организаций $ORGS, снимок $AFTER_ID"
