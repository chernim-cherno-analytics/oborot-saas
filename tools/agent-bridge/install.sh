...
CONFIG_HOME="$("$SCRIPT_DIR/run.sh" resolve config-home)"
ENV_ONLY="$("$SCRIPT_DIR/run.sh" resolve env-only-keys)"
...
# настроек туда попадают, поэтому файл настроек фон прочитает тот же самый — а
# вот значение, живущее только в переменной окружения этой оболочки, до launchd
# не доедет. Установщик тогда проверяет одно, а фон берёт другое, и расходятся
# они молча — ровно тот случай, ради которого затевался `resolve`.
#
# Почему отказ, а не перенос в plist. Переменная окружения перебивает файл
# настроек: вписанное при установке значение пережило бы любую правку
# config.env — владелец менял бы настройку, а фон продолжал бы своё. Место
# настроек одно, и это файл.
LOST=""
for key in $ENV_ONLY; do
    # Каталог состояния — исключение: его установщик кладёт в plist готовым
    # путём, и фон получает именно тот, который здесь проверен.
    if [ "$key" != "STATE_DIR" ]; then
        LOST="${LOST:+$LOST }$key"
    fi
done
if [ -n "$LOST" ]; then
    cat >&2 <<MSG
Эти настройки заданы только переменными окружения: $LOST
Фоновой задаче они не достанутся — launchd не читает ни вашу оболочку, ни её
rc-файлы, — и она работала бы не с теми значениями, которые здесь проверены.
Установка прервана.

Перенесите их (с префиксом OBOROT_BRIDGE_) в файл настроек:

  $CONFIG_FILE

и повторите. Файл читают все трое одинаково: фон, health и установщик. Каталог
настроек ($CONFIG_HOME) установщик записывает в plist, поэтому фон найдёт
именно этот файл, даже если XDG_CONFIG_HOME у вас нестандартный.
MSG
    exit 1
fi
...
<dict>
    ...
    <dict>
        <!-- Обратных кавычек в этом блоке быть не должно: heredoc не закавычен
             (иначе не подставились бы пути), и текст в них выполнился бы как
             команда прямо при установке.
        ...
        <!-- Каталог настроек — тоже от диспетчера ("resolve config-home"), а не
             «~/.config» на веру. Без этой строки фон с нестандартным
             XDG_CONFIG_HOME читал бы ДРУГОЙ config.env (обычно
             несуществующий) и работал бы на значениях по умолчанию, хотя
             установщик проверил настоящие. Здесь путь до файла, а не сами
             настройки: значения остаются в файле, правка config.env действует
             на фон сразу, и копировать сюда нечего — ни секретов, ни имени
             пользователя, ни пути до интерпретатора (переменная окружения
             перебивает файл, и вписанное здесь пережило бы правку
             OBOROT_BRIDGE_TEST_PYTHON в config.env). Настройки, заданные
             только переменными окружения, установщик до этого места не
             пропускает вовсе. -->
