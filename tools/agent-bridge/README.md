...
| `claude-claim.sh` | **единственный разрешённый способ позвать `claude` руками**: берёт бронь на Pull Request, потом запускает модель |
...
tools/agent-bridge/run.sh claim list          # какие Pull Request заняты и кем
```
...
23.08.2026 первый живой тест нашёл гонку: фон увидел замечания к PR #8 и
запустил `claude`, хотя тот же PR в эту минуту правила ручная сессия из другого
чата. Дубль остановили руками до push. Двое на одной ветке — это встречные
правки одних строк, отклонённый push и коммит поверх чужой незаконченной работы.

Теперь один PR может занять только один исполнитель. Бронь берётся неделимой
файловой блокировкой в `<каталог состояния>/claims`, содержит владельца сессии,
номер PR, SHA, время и срок, и снимается сама — когда работа закончилась, когда
она упала и даже когда процесс убили насовсем (`kill -9`, закрытое окно):
блокировку в этом случае снимает ядро. Разбора `ps` и строки команды здесь нет
специально — pid переиспользуются, а сам разбор это ещё одна гонка.

Диспетчер берёт бронь сам, прямо перед вызовом модели, перепроверяет её и
освобождает в любом исходе. Занятый PR он пропускает и попытку из трёх на это
не тратит: чинить никто не пробовал.

Руками `claude` для правки кода запускается ТОЛЬКО так:

```bash
tools/agent-bridge/claude-claim.sh --pr 8                       # интерактивно
tools/agent-bridge/claude-claim.sh --pr 8 --sha <SHA> -- -p "..."   # разовая правка
```

Занято — `claude` не стартует, код возврата 75. Остальные команды:

```bash
tools/agent-bridge/run.sh claim list                 # какие PR заняты и кем
tools/agent-bridge/run.sh claim check --pr 8         # 0 — свободен, 75 — занят
tools/agent-bridge/run.sh claim acquire --pr 8       # взять и выйти (снимать вручную)
tools/agent-bridge/run.sh claim release --pr 8       # снять свою бронь
tools/agent-bridge/run.sh claim release --pr 8 --force   # снять чужую зависшую
```

`--force` нужен, когда владелец завис. Он его НЕ останавливает: сначала
остановите процесс, иначе снятая бронь вернёт двух исполнителей.

Срок брони (`OBOROT_BRIDGE_CLAIM_TTL`, по умолчанию 5400 секунд) — страховка
для случая, когда живость владельца доказать нечем: отсоединённая бронь или
потерянный файл. У живого процесса срок бронь не отбирает: истёкший срок значит
«работа затянулась», а не «работы нет».

Любой сбой самого механизма — недоступный каталог, повреждённая запись, не
взятая блокировка — считается занятостью, и модель не зовётся. Пропуск стоит
минуты до следующего цикла; лишний запуск стоит двух исполнителей на ветке.
...
* `claims/` — брони на Pull Request: `pr-N.json` (кто, что и до какого срока) и
  `pr-N.alive` (файл, которым владелец доказывает, что он ещё жив);
...
Pull Request, не зовёт модель для занятого другой сессией Pull Request,
останавливается после трёх попыток на один SHA, откатывает любые
